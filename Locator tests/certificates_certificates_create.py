...
try:
    ...
    browser.find_element_by_xpath("//button[contains(text(),'Создать шаблон сертификата')]").click()
    course = browser.find_element_by_css_selector('input[data-placeholder="Начните вводить название"]')
    course.send_keys("Как делать контент для YouTube")
    browser.find_element_by_xpath("//span[contains(text(),'Как делать контент для YouTube')]").click()
